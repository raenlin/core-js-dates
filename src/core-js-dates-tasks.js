...
function getNextFriday(date) {
  const newDate = new Date(date);
  const curDay = newDate.getDay();
  let newDay;
  if (curDay < 5) newDay = 5 - curDay;
  if (curDay === 5) newDay = 7;
  if (curDay === 6) newDay = 6;
  const curDate = newDate.getDate();
  newDate.setDate(curDate + newDay);
  return newDate;
}
...
function getCountDaysInMonth(month, year) {
  const newdDate = new Date(year, month - 1, 33);
  return 33 - newdDate.getDate();
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  const res = (date2Ms - date1Ms) / 1000 / 60 / 60 / 24 + 1;
  return res;
}
...
function isDateInPeriod(date, period) {
  ...
  if (curMs >= startDateMs && curMs <= endDateMs) {
    return true;
  }
  return false;
}
